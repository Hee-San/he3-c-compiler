...
assert 105 'main() { return (1+(2+3)*4)*5; }'
assert 10 'main() { return -3*5+25; }'
...
assert 5 'main() { if (0) a=3; else a=5; return a; }'
assert 3 'main() { if (1) a=3; else a=5; return a; }'
assert 7 'main() { if (0) return 3; else if (0) return 5; else return 7; }'
...
assert 36 'main() { return add8(1,2,3,4,5,6,7,8); }'
...
assert 5 'main() { x=3; y=5; return *(&x+16); }'
assert 3 'main() { x=3; y=5; return *(&y-16); }'
assert 5 'main() { x=3; y=&x; *y=5; return x; }'
assert 7 'main() { x=3; y=5; *(&x+16)=7; return y; }'
